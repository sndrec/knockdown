...
function ENT:Think()
	...
	if self.thrusterOwner then
		if not IsValid(self.thrusterOwner) then
			self.thrusterOwner = nil
		end
		if self:IsOn() and self.thrusterOwner:GetThrusterMeter() > 0 then
			self.thrusterOwner:SetThrusterMeter(self.thrusterOwner:GetThrusterMeter() - (0.2 * FrameTime() * self.force))
			self.thrusterOwner.lastThrust = CurTime()
		end
		if self.thrusterOwner:GetThrusterMeter() < 0.5 then
			self:Switch(false)
		end
	end
